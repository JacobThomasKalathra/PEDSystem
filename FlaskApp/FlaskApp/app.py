...
def server_monitoring():
   ...
   PATH='/tmp/chkServers/chkServersBasicStatus'
   def listToString(s): 
   ...
   if os.path.isfile(PATH) and os.access(PATH, os.R_OK):
    ...
    myfile = open("/tmp/chkServers/chkServersBasicStatus")
    #print(myfile.read())
    myfile.close()
    file = open('/tmp/chkServers/chkServersBasicStatus')
    merror = FileCheck('/tmp/chkServers/same_error_count')
    session['merr'] = merror
    for line in file.readlines():
       ...
       if mycontent == "SUCCESS":
         status = "SUCCESS"
         session['mStatus'] =1
         os.system("cat /tmp/chkServers/chkServersBasic_*.success > merged.html")
         f = open("merged.html", "r")
	 text = f.read()
	 #print text
         f.close()
         successFile =[]
         successFile = glob.glob("/tmp/chkServers/chkServersBasic_*.success")
         #print (type(successFile))
        
         NameofFile=  successFile[0]
         NameofFile = NameofFile[16:47:]
	 ...
	 report_time = datetime.datetime.strptime(report_time, '%Y%m%d%H%M%S').strftime('%d/%m/%Y %H:%M:%S')
         ...
         #Logfile = glob.glob("/tmp/chkServers/chkServersBasic_*.Error")
         Logfile = os.system("ls  /tmp/chkServers/summary/*.Error | sed -e 's/\..*$//' > Logfile.txt")
         ...
         with open('Logfile.txt') as f:
              ...
              while line:
                  ...
                  if line[35:50:] != '':
                     result[line[35:50:]]= get_log(line)
         ...
         os.system("cat /tmp/chkServers/chkServersBasic_*.error > merged.html")
         f = open("merged.html", "r")
	 text = f.read()
	 #print text
         f.close()
         successFile =[]
         successFile = glob.glob("/tmp/chkServers/chkServersBasic_*.Error")
         #print (successFile)
